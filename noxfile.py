...
if not VENV_DIR.exists() and not ON_CI:
    ...
    nox.options.sessions = ["test", "doctest", "coverage", "lint", "docs"]


def poetry_install(session: nox.Session, *args: str, **kwargs: dict[str, Any]) -> None:
    """
...
def set_up_vscode(session: nox.Session) -> None:
    ...
    if not VSCODE_DIR.exists():
        ...
        with SETTINGS_JSON.open(mode="w", encoding="utf-8") as f:
...
def doctest(session: nox.Session) -> None:
    """
    Runs doctest against the module docstrings.
    """
    session_requires(session, "poetry")

    requirements = SESSION_REQUIREMENTS.get("doctest", [])
    if not requirements:
        session.error(
            "Requirements for nox session: 'doctest', not found in noxfile.py"
        )
...
def deploy_docs(session: nox.Session) -> None:
    ...
    update_seeds(session)
    poetry_install(session, *requirements)  # type: ignore[misc]
